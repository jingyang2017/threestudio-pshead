...
class FRMeter: 
    ...
    def score_from_path(self, img1_path, img2_path):
        ...
        if len(faces)==1:
            ...
            return torch.nn.functional.cosine_similarity(faceid_embeds_1,faceid_embeds_2).item()
...
def score_from_method_for_dataset_yj(my_scorer,
    ...
    # examples = os.listdir(input_path) #"*_rgba.png"
    examples = glob.glob(input_path+'/*_rgba.png')
    examples.sort()

    if method=='ours':
        img_list=["./ours/results/000121/{000121_rgba.png}@20240905-160527/save/test/images",
                  "./ours/results/004862/{004862_rgba.png}@20240909-222144/save/test/images",
                  "./ours/results/007672/{007672_rgba.png}@20240906-093505/save/test/images",
                  "./ours/results/000362/{000362_rgba.png}@20240906-081502/save/test/images",
                  "./ours/results/019679/{019679_rgba.png}@20240904-002328/save/test/images",
                  "./ours/results/jing/{jing_rgba.png}@20240905-230523/save/test/images",
                  "./ours/results/kyle/{kyle_rgba.png}@20240906-002409/save/test/images" ,
                  "./ours/results/subject1/{subject1_rgba.png}@20240909-133126/save/test/images",
                  "./ours/results/subject2/{subject2_rgba.png}@20240906-025928/save/test/images",
                  "./ours/results/subject3/{subject3_rgba.png}@20240906-041902/save/test/images"] 
        img_list.sort()
        ref_index = 121   
    elif method=='oursv2':
        img_list = glob.glob("./ours_f2b/**/*/images",recursive=True)
        # img_list=["./ours/results/000121/{000121_rgba.png}@20240905-160527/save/test/images",
        #           "./ours/results/004862/{004862_rgba.png}@20240909-222144/save/test/images",
        #           "./ours/results/007672/{007672_rgba.png}@20240906-093505/save/test/images",
        #           "./ours/results/000362/{000362_rgba.png}@20240906-081502/save/test/images",
    ...
    for i in range(len(examples)):
        ...
        if score_type in ['clip','cl','fr']:
            ...
            if score_type=='fr':
                if method == 'oursv2':
                    temp = novel_view[:30]
                    temp.extend(novel_view[-30:])
                elif method=='dreamgaussian':
                    temp = novel_view[90:150]
                elif method =="erad3d":
                    temp = novel_view[:14]
                    temp.extend(novel_view[-14:])
                elif method == "panohead":
                    temp = novel_view[:30]
                    temp.extend(novel_view[-30:])
                elif method == 'magic123':
            ...
            if method == "magic123":
                temp = glob.glob(f"{img_list[i]}/*lambertian.jpg")
                temp.sort()
            else:
                temp = glob.glob(f"{img_list[i]}/*.png")
                temp.sort()

            if ref_index is not None:
                # print(img_list[i])
                # print(ref_index)
                novel_view = [temp[ref_index]]
            else:
                novel_view = [ref_path.replace("target","proj")]
            print(f'[INOF] {score_type} loss for example {examples[i]} between {ref_path} and {novel_view}')
        # breakpoint()
        score_i = my_scorer.score_gt([ref_path], novel_view)
        scores[examples[i]] = score_i
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", type=str, default='all_metrics/results21')
    args = parser.parse_args()

    clip_scorer = CLIP(args.device)
    lpips_scorer = LPIPSMeter(device=args.device)
    ...
    results_dict['clip'] = score_from_method_for_dataset_yj(
        clip_scorer, args.method, 'clip')
    
    results_dict['psnr'] = score_from_method_for_dataset_yj(
        psnr_scorer, args.method,  'psnr')
    
    results_dict['lpips'] = score_from_method_for_dataset_yj(
        lpips_scorer, args.method,  'lpips')
    
    results_dict['cl'] = score_from_method_for_dataset_yj(
        cl_scorer, args.method, 'cl')

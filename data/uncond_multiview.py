...
@dataclass
class FixMultiviewCameraDataModuleConfig:
    height: Any = 64
    width: Any = 64
    batch_size: Any = 1
...
class FixMultiviewCameraIterableDataset(IterableDataset, Updateable):
    def __init__(self, cfg: Any) -> None:
        super().__init__()
        self.cfg: FixMultiviewCameraDataModuleConfig = cfg
...
class MultiviewCameraDataset(Dataset):
    def __init__(self, cfg: Any) -> None:
        ...
        # We did not use it
        # print(self.elevation_range)
        if self.elevation_range[1] == self.elevation_range[0]:
            # sample elevation angles uniformly with a probability 0.5 (biased towards poles)
            elevation_deg = (
        ...
        azimuth_deg = (
            torch.zeros(real_batch_size).reshape(-1, 1)
            + torch.arange(self.cfg.n_view).reshape(1, -1)
        ).reshape(-1) / self.cfg.n_view * (
            self.azimuth_range[1] - self.azimuth_range[0]
        ) + self.azimuth_range[
        ...
        * (self.camera_distance_range[1] - self.camera_distance_range[0])
        ...
        zoom: Float[Tensor, "B"] = (
            torch.zeros(real_batch_size) * (self.zoom_range[1] - self.zoom_range[0])
            + self.zoom_range[0]
        ).repeat_interleave(self.cfg.n_view, dim=0)
        ...
        camera_perturb: Float[Tensor, "B 3"] = (
            torch.zeros(real_batch_size, 3) * 2 * self.cfg.camera_perturb
            - self.cfg.camera_perturb
        ).repeat_interleave(self.cfg.n_view, dim=0)
        ...
        center_perturb: Float[Tensor, "B 3"] = (
            torch.zeros(real_batch_size, 3) * self.cfg.center_perturb
        ).repeat_interleave(self.cfg.n_view, dim=0)
...
class FixMultiviewCameraDataModule(pl.LightningDataModule):
    cfg: FixMultiviewCameraDataModuleConfig

    def __init__(self, cfg: Optional[Union[dict, DictConfig]] = None) -> None:
        super().__init__()
        self.cfg = parse_structured(FixMultiviewCameraDataModuleConfig, cfg)

    def setup(self, stage=None) -> None:
        if stage in [None, "fit"]:
            self.train_dataset = FixMultiviewCameraDataModuleConfig(self.cfg)
        if stage in [None, "fit", "validate"]:
            self.val_dataset = FixMultiviewCameraDataModuleConfig(self.cfg)
        if stage in [None, "test", "predict"]:
            self.test_dataset = FixMultiviewCameraDataModuleConfig(self.cfg)
